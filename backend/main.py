...
# NEW: Initialize SMTP settings for email
SMTP_SERVER = os.getenv("SMTP_SERVER", "smtp.gmail.com")
SMTP_PORT = int(os.getenv("SMTP_PORT", 587))
SMTP_USERNAME = os.getenv("SMTP_USERNAME")
SMTP_PASSWORD = os.getenv("SMTP_PASSWORD")

# NEW: Utility function to send email
async def send_confirmation_email(
    ...
    try:
        ...
        Your appointment with {doctor_name} ({doctor_specialty}) has been successfully booked.
        ...
        Thank you for using our platform. Please arrive 10 minutes early for your appointment.
...
async def upload_to_s3(file_bytes: bytes, file_name: str) -> str:
    try:
        ...
        s3_url = f"https://{S3_BUCKET_NAME}.s3.{os.getenv('AWS_REGION')}.amazonaws.com/{file_name}"
        return s3_url
...
async def predict_xray(file: UploadFile = File(...)):
    ...
    s3_file_name = f"xray/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
    ...
    try:
        ...
        return JSONResponse(content={"predictions": predictions, "s3_url": s3_url})
    except Exception as e:
        os.remove(temp_path)
...
async def generate_report(
    ...
    file.file.seek(0)
    file_bytes = await file.read()
    s3_file_name = f"{modality}/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
    ...
    try:
        ...
        return JSONResponse(content={"symptoms": symptoms, "disease": disease, "report": report, "s3_url": s3_url})
...
async def generate_report_ct2d(file: UploadFile = File(...)):
    ...
    file.file.seek(0)
    file_bytes = await file.read()
    s3_file_name = f"ct2d/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
...
async def generate_report_ct3d(file: UploadFile = File(...)):
    ...
    file.file.seek(0)
    file_bytes = await file.read()
    s3_file_name = f"ct3d/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
    ...
    try:
        ...
        return JSONResponse({**latest_reports["ct3d"], "s3_url": s3_url})
    except Exception as e:
        if os.path.exists(temp_path): os.remove(temp_path)
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_report_mri3d(file: UploadFile = File(...)):
    ...
    file.file.seek(0)
    file_bytes = await file.read()
    s3_file_name = f"mri3d/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
    ...
    try:
        ...
        return JSONResponse({**latest_reports["mri3d"], "s3_url": s3_url})
    except Exception as e:
        if os.path.exists(temp_path): os.remove(temp_path)
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_report_ultrasound(file: UploadFile = File(...)):
    ...
    file.file.seek(0)
    file_bytes = await file.read()
    s3_file_name = f"ultrasound/{file.filename}"
    s3_url = await upload_to_s3(file_bytes, s3_file_name)
    ...
    try:
        ...
        return JSONResponse(
            content={"symptoms": symptoms, "disease": disease, "report": report, "s3_url": s3_url}
        )
    except HTTPException:
...
# NEW: Pydantic model for appointment data
...
# NEW: Endpoint to handle appointment booking
...
async def book_appointment(request: AppointmentRequest):
    try:
        ...
        return JSONResponse(content={"message": "Appointment booked successfully. Confirmation email sent."})
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
